function onMostraClick(event) {   
    ...
    mostra.textContent="▼ Ingredienti";
    fetch("Controllo.php?em="+encodeURIComponent(String(pa))+"&t=Mostra").then(fetchResponse).then(jsonMostra);
...
function jsonMostra(json) {
    ...
    for(let divs of div) {
        if(divs.dataset.name==json.item[0].ID){
            ...
            newDesc.textContent=json.item[0].ingredienti;
...
function onRecensioniClick(event) {
    ...
    fetch("Controllo.php?em="+encodeURIComponent(String(pa))+"&p=NULL&t=Recensioni").then(fetchResponse).then(jsonRecensioni);
...
function InviaRecensione(event) {
    ...
    fetch("Controllo.php?em="+encodeURIComponent(String(area.value))+"&t=ADDRecensioni"+"&u="+encodeURIComponent(String(utente))+"&p="+encodeURIComponent(String(piatto))+"&t=ADDRecensioni").then(fetchResponse).then(jsonADDRecensione);
}

function jsonADDRecensione(json) {
    fetch("Controllo.php?em="+encodeURIComponent(String(json.recensione.piatto))+"&p=NULL&t=Recensioni").then(fetchResponse).then(jsonReloadRec);   
}
...
function OnClickcancella(event) {
    ...
    fetch("Controllo.php?&em="+encodeURIComponent(String(piatto))+"&c="+encodeURIComponent(String(idRec))+"&p=cancella&t=Recensioni").then(fetchResponse).then(jsonReloadRec);
}
...
function redHeart(event) {
    ...
    red.src="./img/cuore red.png";
    fetch("Controllo.php?em="+encodeURIComponent(String(input.value))+"&t=Like").then(fetchResponse).then(jsonLike);
...
function blackHeart(event) {
    let input=event.currentTarget;
    let red=input.parentNode.querySelector('.imgheart');
    red.src="./img/cuore black.png";
    fetch("Controllo.php?em="+encodeURIComponent(String(input.value))+"&t=UnLike").then(fetchResponse).then(jsonUnLike);
    red.removeEventListener('click',blackHeart);
    red.addEventListener('click',redHeart);
}

function jsonLike(json) {
    fetch("Controllo.php?em=Esegui&t=Dati").then(fetchResponse).then(jsonReload);    
}
...
function EliminaPost(event) {
    ...
    fetch("Controllo.php?em="+encodeURIComponent(String(id))+"&t=Elimina").then(fetchResponse).then(jsonCancella);
}
...
function NewPost() {
    document.location.href = "newPost.php";        
}
...
function jsonDati(json) {
    ...
    for (let divs of json.item) {
            ...
            newMostra.textContent="▶ Ingredienti";
...
fetch("Controllo.php?em=Esegui&t=Dati").then(fetchResponse).then(jsonDati);