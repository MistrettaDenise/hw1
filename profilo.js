function PostLike(json)  {
    ...
    intestazione.textContent="I like che hai messo";
...
function PostRecensioni(json) {
    ...
    let intestazione= document.createElement('h1');
    intestazione.textContent="Le recensioni che hai fatto";
...
function ButtonmodificaPassword(event) {
    let vecchiapassword;
    fetch('controllo.php', {
        method: 'POST',
        body: updateRequest
...
function modificaPassword(json) {
    ...
    let intestazione= document.createElement('h1');
    intestazione.textContent="Modifica la tua password";
...
fetch("Controllo.php?em=Esegui&t=Profilo").then(fetchResponse).then(jsonProfilo);