...
function caricaPiatto(event) {
    ...
    let ingredienti=event.currentTarget.parentNode.querySelector('.conteiner .ingredienti textarea').value;
    fetch("Controllo.php?em="+encodeURIComponent(String(img))+"&tit="+encodeURIComponent(String(titolo))+"&desc="+encodeURIComponent(String(ingredienti))+"&t=caricaPiatto").then(fetchResponse).then(jsonCaricato);
...
intestazione.textContent="Inserisci di dati della nuova pizza!";
...
DescTesto.textContent="Ingredienti";
...
divCaricaDesc.classList.add('ingredienti');
let inviaDati=document.createElement('button');
inviaDati.textContent='Aggiungi pizza!';
...
testo.textContent='Anteprima pizza';
